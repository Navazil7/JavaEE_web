                    ...
                    <div class="box-body">
                        <form class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-lg-5">
                                    <img id="qimg" src="${user.upic}" style="height: 150px;width: 150px;border: rgba(121,121,121,0.52) 5px solid" class="img-circle" alt="User Image">
                                </div>
                            </div>
                                ...
                                <c:forEach var="attraction" items="${attractions}" varStatus="status">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="post-preview">
                                            <br>
                                            <c:if test="${attraction.attr_status=='未使用'}" var="judge">
                                                <c:set var="color" value="red"></c:set>
                                            </c:if>
                                            <c:if test="${!judge}">
                                                <c:set var="color" value="green"></c:set>
                                            </c:if>
                                            <h3 style="color: ${color}" id="state${status.count}">${attraction.attr_status}！</h3>
                                            <a href="/view/content?tpVid=${attraction.attr_vid}">
                                                <p id="oid${status.count}">订单号:${attraction.attr_orderId}</p>
                                                <p id="uid${status.count}">订单用户Id:${attraction.attr_uid}</p>
                                                <p id="name${status.count}">景点名称:${attraction.attr_name}</p>
                                                <p id="place${status.count}">景点地点:${attraction.attr_place}</p>
                                                <p id="email${status.count}">订单用户邮箱:${attraction.attr_uemail}</p>
                                                <p id="phone${status.count}">订单用户手机号码:${attraction.attr_uphone}</p>
                                                <p id="cost${status.count}">订单金额:￥${attraction.attr_cost}元</p>
                                                <p id="time${status.count}">订单创建时间:${attraction.attr_createDate}  ${attraction.attr_createTime}</p>
                                                <p id="day${status.count}">订单时效:${attraction.attr_time}</p>
                                                <p id="num${status.count}">订单数量:${attraction.attr_num}</p>
                                            </a>
                                                                ...
                                                                if (cpnt == "usebtn") {
                                                                    ...
                                                                    if (color === 'red') {
                                                                        ...
                                                                        layer.alert("使用成功！", {icon: 1}, function () {
                                                                            window.location.href = '/order/update?key=attraction&tpoid=' + oid + '&tpuid=' + uid;
                                                                        });
