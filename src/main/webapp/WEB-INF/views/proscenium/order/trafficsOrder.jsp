                    ...
                    <div class="box-body">
                        <form class="form-horizontal">
                            ...
                            <div class="center-block">
                                <c:forEach var="traffic" items="${traffics}" varStatus="status">
                                    <c:set var="total" value="${status.count}"></c:set>
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="post-preview">
                                            <br>
                                            <c:if test="${traffic.traffic_status=='未使用'}" var="judge">
                                                <c:set var="color" value="red"></c:set>
                                            </c:if>
                                            <c:if test="${!judge}">
                                                <c:set var="color" value="green"></c:set>
                                            </c:if>
                                            <h3 style="color: ${color}" id="state${status.count}">${traffic.traffic_status}！</h3>
                                            <a href="#">
                                                <p id="oid${status.count}">订单号:${traffic.traffic_orderId}</p>
                                                <p id="uid${status.count}">订单用户Id:${traffic.traffic_uid}</p>
                                                <p id="type${status.count}">交通类型:${traffic.traffic_type}</p>
                                                <p id="email${status.count}">订单用户邮箱:${traffic.traffic_uemail}</p>
                                                <p id="phone${status.count}">订单用户手机号码:${traffic.traffic_uphone}</p>
                                                <p id="depart${status.count}">出发地:${traffic.traffic_depart}</p>
                                                <p id="dest${status.count}">目的地：${traffic.traffic_des}</p>
                                                <p id="depart_time${status.count}">出发时间:${traffic.traffic_departDate}  ${traffic_departTime}</p>
                                                <p id="dest_time${status.count}">到达时间:${traffic.traffic_desDate}  ${traffic_desTime}</p>
                                                <p id="cost${status.count}">订单金额:￥${traffic.traffic_cost}元</p>
                                                <p id="time${status.count}">订单创建时间:${traffic.traffic_createDate}  ${traffic.traffic_createTime}</p>
                                                <p id="num${status.count}">订单数量:${traffic.traffic_num}</p>
                                            </a>
                                        ...
                                        <div>
                                            ...
                                            <div class="col-lg-11">
                                                <br>
                                                    <%--                                                <i class="btn btn-danger btn-xs fa" id="display${status.count}" style="background-color: darkblue">显示二维码</i>--%>
                                                ...
                                                <div class="demo" style="display: none;width: 200px;text-align: center" id="shareDiv${status.count}">
                                                                ...
                                                                if (cpnt == "usebtn") {
                                                                    ...
                                                                    if (color === 'red') {
                                                                        ...
                                                                        layer.alert("使用成功！", {icon: 1}, function () {
                                                                            window.location.href = '/order/update?key=traffic&tpoid=' + oid + '&tpuid=' + uid;
                                                                        });
                                                ...
                                                <%--右边--%>
                                            <div class="col-lg-1">
                                                <i class="btn btn-danger btn-xs fa fa-trash" onclick="isDelete('${traffic.traffic_uid}','${traffic.traffic_orderId}')"></i>
                                            </div>
                                            <br>
                                        </div>
                                        <br>
